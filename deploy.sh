...
# ============================================================================
# nanobot-custom 自动化部署脚本 (增强版)
# 版本: 3.0.0
# 功能: 自动同步源码 + 自定义技能 + 安全检查 + 自动部署 + 生成报告
# ============================================================================
...
# 自定义技能目录 (源码)
SKILLS_DIR="$HOME/.nanobot/workspace/skills"
# 自定义技能目标目录 (部署工作区)
CUSTOM_SKILLS_DIR="$HOME/.nanobot/workspace/nanobot-custom/custom-skills"
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "  --tag VERSION 创建指定版本的标签"
                echo "  --skip-sync   跳过源码同步（手动同步后使用）"
                echo "  -h, --help    显示帮助信息"
                exit 0
                ;;
            *)
...
sync_source_code() {
    if [ "$SKIP_SYNC" = true ]; then
        print_info "已跳过源码同步（--skip-sync 参数）"
        return 0
    fi
    ...
    print_info "目标目录：$HOME/.nanobot/workspace/nanobot-custom/nanobot"
    
    # 检查源目录是否存在
    if [ ! -d "$SOURCE_DIR" ]; then
        print_error "源目录不存在：$SOURCE_DIR"
        print_warning "请确保 nanobot 源码安装在正确位置"
        print_info "可以使用 --skip-sync 跳过此步骤"
        return 1
    fi
    
    # 创建备份
    TARGET_DIR="$HOME/.nanobot/workspace/nanobot-custom/nanobot"
    BACKUP_DIR="$HOME/.nanobot/workspace/nanobot-custom/backups/sync_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$BACKUP_DIR"
    ...
    print_success "同步完成!"
    print_info "源目录 Python 文件数：$source_files"
    print_info "目标目录 Python 文件数：$target_files"
...
# 同步自定义技能函数
sync_custom_skills() {
    print_header "🔄 同步自定义技能"
    
    print_info "源目录：$SKILLS_DIR"
    print_info "目标目录：$CUSTOM_SKILLS_DIR"
    
    # 检查源目录是否存在
    if [ ! -d "$SKILLS_DIR" ]; then
        print_warning "自定义技能源目录不存在：$SKILLS_DIR"
        print_info "如果没有自定义技能，可以忽略此警告"
        return 0
    fi
    ...
    # 创建备份 (如果目标目录已有内容)
    if [ "$(ls -A "$CUSTOM_SKILLS_DIR" 2>/dev/null)" ]; then
        BACKUP_DIR="$HOME/.nanobot/workspace/nanobot-custom/backups/skills_$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$BACKUP_DIR"
        print_info "创建技能备份到：$BACKUP_DIR"
        cp -r "$CUSTOM_SKILLS_DIR"/* "$BACKUP_DIR"/ 2>/dev/null || true
    fi
    
    # 使用 rsync 同步
    print_info "正在同步自定义技能..."
    rsync -av --exclude '__pycache__' --exclude '*.pyc' \
        --exclude '*.pyo' --exclude '.git' \
        "$SKILLS_DIR"/ "$CUSTOM_SKILLS_DIR"/
    
    # 统计
    source_skills=$(find "$SKILLS_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l)
    target_skills=$(find "$CUSTOM_SKILLS_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l)
    
    print_success "自定义技能同步完成!"
    print_info "源目录技能数：$source_skills"
    print_info "目标目录技能数：$target_skills"
...
main() {
    ...
    print_header "🔧 nanobot-custom 部署工具 v3.0.0 (增强版)"
    ...
    # 步骤 1: 同步主源码
    if ! sync_source_code; then
        print_warning "同步失败，但将继续部署（使用现有代码）"
    fi
    
    # 步骤 2: 同步自定义技能
    if ! sync_custom_skills; then
        print_warning "自定义技能同步失败，但将继续部署"
    fi
